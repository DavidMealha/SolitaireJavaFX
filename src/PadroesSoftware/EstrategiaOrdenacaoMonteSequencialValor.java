 ...
 * @author David
 */
public class EstrategiaOrdenacaoMonteSequencialValor implements EstrategiaOrdenacaoMonteSequencial {
    ...
    public boolean adicionarCarta(Carta carta, Stack<Carta> monteDeCartas) {
        ...
        if(!monteDeCartas.isEmpty()){
            ...
            //Retorn valor do naipe da ultima carta do monte
            valorNaipeUltimaCarta = ultimaCarta.getValorNaipe();
        }
        
        if (monteDeCartas.isEmpty()) {
            monteDeCartas.push(carta);
            return true;
        } else if (((valorNaipeUltimaCarta == 2 || valorNaipeUltimaCarta == 4))
                && (carta.getValorNaipe() == 1 || carta.getValorNaipe() == 3)
                && (valorFaceUltimaCarta == carta.getValorFace() + 1)) {
            
            monteDeCartas.push(carta);
            return true;

        } else if(((valorNaipeUltimaCarta == 1 || valorNaipeUltimaCarta == 3))
                && (carta.getValorNaipe() == 2 || carta.getValorNaipe() == 4)
                && (valorFaceUltimaCarta == carta.getValorFace() + 1)) {
